...
class GymbieRegister extends StatefulWidget {
  ...
  const GymbieRegister(
      {super.key, this.type = 'register', this.userId, this.kakaoToken});
...
class _GymbieRegisterState extends State<GymbieRegister> {
  ...
  Future<void> onClickConfirmButton() async {
    if (widget.type == 'register') {
      ...
      params['user_phone_number'] = _phoneNumber!;
      params["user_birthday"] = _birthday!.toIso8601String();
      params["user_gender"] = _gender;
      params['kakao_token'] = widget.kakaoToken!;
      UserAuth userAuth = await authRepository.signUpUser(params);
